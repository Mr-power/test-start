...
public class LogRecordOperationSource {
    ...
    private Collection<LogRecordOps> parseLogRecords(AnnotatedElement annotatedElement) {
        // 获取合并后logRecord 的注解
        Collection<LogRecordAnnotation> logRecords = AnnotatedElementUtils.getAllMergedAnnotations(annotatedElement, LogRecordAnnotation.class);
        Collection<LogRecordOps> result = null;
        if (!CollectionUtils.isEmpty(logRecords)) {
            result = lazyInit(result);
            for (LogRecordAnnotation logRecord : logRecords) {
     ...
     * @return
     */
    private LogRecordOps parseLogRecord(AnnotatedElement annotatedElement, LogRecordAnnotation logRecord) {
